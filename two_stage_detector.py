...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        # Pass the dummy input through the backbone network
        dummy_output = self.backbone(dummy_input)
        # Get the number of channels for the output feature maps
        feature_channels = {"c3": dummy_output["c3"].size(1),"c4": dummy_output["c4"].size(1),"c5": dummy_output["c5"].size(1)}
        # Initialize lateral convolutions
        self.fpn_params["lateral_p5"] = nn.Conv2d(feature_channels["c5"], out_channels, kernel_size=1, stride=1, padding=0)
        self.fpn_params["lateral_p4"] = nn.Conv2d(feature_channels["c4"], out_channels, kernel_size=1, stride=1, padding=0)
        self.fpn_params["lateral_p3"] = nn.Conv2d(feature_channels["c3"], out_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, images: torch.Tensor):
        ...
        # Apply lateral convolution to c5 feature map
        lateral5 = self.fpn_params["lateral_p5"](backbone_feats["c5"])
        fpn_feats["p5"] = self.fpn_params["out_p5"](lateral5)

        # Apply lateral convolution to c4 feature map
        lateral4 = self.fpn_params["lateral_p4"](backbone_feats["c4"])
        # Upsample c5 feature map and add to c4 lateral output
        combined4 = lateral4 + F.interpolate(lateral5, scale_factor=2, mode="nearest")
        output4 = self.fpn_params["out_p4"](combined4)
        fpn_feats["p4"] = output4

        # Apply lateral convolution to c3 feature map
        lateral3 = self.fpn_params["lateral_p3"](backbone_feats["c3"])
        # Upsample c4 feature map and add to c3 lateral output
        combined3 = lateral3 + F.interpolate(lateral4, scale_factor=2, mode="nearest")
        output3 = self.fpn_params["out_p3"](combined3)
        fpn_feats["p3"] = output3
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels)):
            convolution = nn.Conv2d(in_channels if i == 0 else stem_channels[i-1],stem_channels[i],kernel_size=3,stride=1,padding=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level in ["p3", "p4", "p5"]:
          xinput = feats_per_fpn_level[level]
          kin = self.stem_rpn(xinput)
          out = self.pred_obj(kin)
          bact = self.pred_box(kin)
          bsize, num_anchors, H, W = bact.size()
          out = out.reshape(out.shape[0],-1).to(xinput.device)
          bact = bact.reshape(bact.shape[0],-1).unsqueeze(2).permute(0,1,2).to(xinput.device)
          bact = bact.view(bsize, -1, 4)
          object_logits[level] = out
          boxreg_deltas[level] = bact
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        # Replace "pass" statement with your code
        B, C, H, W = feat_shape 
        pointc = level_stride // 2
        intx = torch.arange(W, dtype=dtype, device=device) * level_stride + pointc 
        inty = torch.arange(H, dtype=dtype, device=device) * level_stride + pointc 
        intxx = torch.zeros((intx.size(dim=0),intx.size(dim=0)))
        intyy = torch.zeros((inty.size(dim=0),inty.size(dim=0)))
        inxx,inyy = torch.meshgrid(intx,inty)
        c = torch.stack([inxx,inyy],dim=-1)
        c = c.reshape(-1,2)
        location_coords[level_name] = c
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            ##################################################################
            # Replace "pass" statement with your code
            tensora = torch.tensor(aspect_ratio)
            ta = (stride_scale * level_stride) ** 2
            width = torch.sqrt(ta / tensora)
            heigth = ta / width
            xx = locations - 0.5 * torch.tensor([width, heigth])
            xy = locations + 0.5 * torch.tensor([width, heigth])
            anchor_boxes.append(torch.stack([xx[:, 0], xx[:, 1], xy[:, 0], xy[:, 1]], dim=1))
            ##################################################################
            #                           END OF YOUR CODE                     #
            ##################################################################
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    coordmax = torch.min(boxes1[:, None, 2:], boxes2[:, 2:])
    coordmin = torch.max(boxes1[:, None, :2], boxes2[:, :2])
    inter = torch.clamp(coordmax - coordmin, min = 0)
    aint = inter.prod(dim=2)
    area1 = (boxes1[:, 2] - boxes1[:, 0]) * (boxes1[:, 3] - boxes1[:, 1])
    area2 = (boxes2[:, 2] - boxes2[:, 0]) * (boxes2[:, 3] - boxes2[:, 1])
    auni = area1[:, None] + area2 - aint
    iou = aint / auni
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rcnn_get_deltas_from_anchors(
    ...
    anchor_cx = anchors[:, 0] + 0.5 * anchor_width
    anchor_cy = anchors[:, 1] + 0.5 * anchor_height

    # Compute ground truth box dimensions
    ...
    delta_x = (gt_cx - anchor_cx) / anchor_width
    delta_y = (gt_cy - anchor_cy) / anchor_height
    delta_w = torch.log(gt_width / anchor_width)
    delta_h = torch.log(gt_height / anchor_height)

    # Identify background/neutral ground truth boxes
    bg_mask = (gt_boxes == -1).any(dim=1)
    bg_indices = torch.nonzero(bg_mask).flatten()

    # Set deltas for background/neutral boxes to a large negative value
    deltas[bg_indices, :] = torch.tensor([-1e8, -1e8, -1e8, -1e8], device=deltas.device)

    # Set deltas for foreground boxes
    fg_mask = ~bg_mask
    fg_indices = torch.nonzero(fg_mask).flatten()
    deltas[fg_indices, 0] = delta_x[fg_mask]
    deltas[fg_indices, 1] = delta_y[fg_mask]
    deltas[fg_indices, 2] = delta_w[fg_mask]
    deltas[fg_indices, 3] = delta_h[fg_mask]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rcnn_apply_deltas_to_anchors(
    ...
    deltaw = deltas[:, 2]
    deltah = deltas[:, 3]
    # Apply delta values to anchors
    xcpred = deltax * w + xc
    ycpred = deltay * h + yc
    widthpred = torch.exp(deltaw) * w
    heightpred = torch.exp(deltah) * h
    # Compute coordinates of predicted boxes
    x1 = xcpred - 0.5 * widthpred
    y1 = ycpred - 0.5 * heightpred
    x2 = xcpred + 0.5 * widthpred
    y2 = ycpred + 0.5 * heightpred
    # Stack predicted coordinates into output tensor
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    boxes_len = len(boxes)
    if scores_len > boxes_len:
      scores = scores[:boxes_len]
    elif boxes_len > scores_len:
      boxes = boxes[:scores_len]
    ...
    K = []
    while True:
    # If there is only one box left, add it and break
      if len(sorted_indices) <= 1:
        if len(sorted_indices) == 1:
            selected_indices.append(sorted_indices[0])
        break
      # Add the highest-scoring box to the selected list
      selected_indices.append(sorted_indices[0])
      # Compute IoU between the highest-scoring box and the remaining boxes
      current_box = boxes[0].unsqueeze(0)
      remaining_boxes = boxes[1:]
      ious = iou(current_box, remaining_boxes)
      # Create a mask to remove boxes with IoU greater than the threshold
      mask = (ious > iou_threshold).logical_not()
      # Update the boxes and indices based on the mask
      sorted_indices = sorted_indices[1:][mask.flatten()]
      boxes = remaining_boxes[mask.unsqueeze(-1).expand_as(remaining_boxes)].reshape(-1, 4)
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        for level, feats in feats_per_fpn_level.items():
          fpnlevel[level] = feats.shape
        coordsl = get_fpn_location_coords(fpnlevel,strides_per_fpn_level)
        anchors_per_fpn_level = generate_fpn_anchors(coordsl,strides_per_fpn_level,stride_scale= self.anchor_stride_scale,aspect_ratios=self.anchor_aspect_ratios)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for i in range (len(gt_boxes)):
          box = gt_boxes[i]
          matched_gt_boxes.append(rcnn_match_anchors_to_gt(anchor_boxes.to(gt_boxes.device),box,self.anchor_iou_thresholds))
        ...
        if self.training:
            ...
            # Replace "pass" statement with your code
            ifg, ibg = sample_rpn_training(matched_gt_boxes,self.batch_size_per_image*num_images,0.5)
            ind = torch.cat([ifg, ibg])
            inda = torch.cat([ifg, ibg])
            boxanchor = anchor_boxes.to(gt_boxes.device)
            objesample = pred_obj_logits[ind]
            sample_pred_deltas = pred_boxreg_deltas[ind]
            sample_anchor = boxanchor[inda]
            sample_gt_box = matched_gt_boxes[ind]
            gt_box_deltas= rcnn_get_deltas_from_anchors(sample_anchor.to(gt_boxes.device), sample_gt_box)
            gt_obj_logits = torch.concat([torch.ones(ifg.shape[0]),torch.zeros(ibg.shape[0])])
            ...
            loss_obj = F.binary_cross_entropy_with_logits(objesample.to(gt_boxes.device), gt_obj_logits.to(gt_boxes.device),reduction="none")
...
class FasterRCNN(nn.Module):
    ...
    def forward(
        ...
        loss_cls = None
        # Replace "pass" statement with your code
        idxgf, idxbg = sample_rpn_training(matched_gt_boxes,num_images,0.25)
        ind = torch.cat([idxgf, idxbg])
        predsample = pred_cls_logits[ind].to(gt_boxes.device)
        gt = matched_gt_boxes[ind].to(gt_boxes.device)
        loss_cls = F.cross_entropy(predsample,(gt[:,4]+1).to(torch.int64))
    ...
    def inference(
        ...
        # Replace "pass" statement with your code
        scoresp, classesp = torch.max(pred_cls_logits,dim=1)
        sm = scoresp > test_score_thresh
        pred_scores = scoresp[sm]
        pred_classes = classesp -1
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
